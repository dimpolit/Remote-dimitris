...
Subroutine Anchovy(p,First,TTime, Tbefore,zoo4,zoo5,zoo6,wtemp)
  ...
  real(8)           :: CTM, CTO,CTOla,CTOj, CQ, RTM, RTO, RQ ! parameters
  real(8)           :: TTime, Tbefore, z4, z5, z6,temp
  real(4)           :: zoo4,zoo5,zoo6,wtemp,wwtemp
...
!     =====================================
    k(1,2)=p(1);k(2,2)=p(2);k(3,2)=p(3);
    k(1,3)=p(4);k(2,3)=p(5); CTOla=p(6);CTOj=p(7);
    Tm=p(8)
!    Tm=18.0
...
!  k (1) = 0.06; k (2) = 0.06; 
  ...
  ! --- to get food energy equivalents of a gram of oxygen respired
   ...
   If  (iage(i) .eq. 2) then
        CTO=CTOla; CTM=25.0;  CQ=2.22;  RTO=28.0; RTM=33.0; RQ=2.22
  elseif  (iage(i) .eq. 3)  then
         CTO=CTOj;CTM=25.0;  CQ=2.22; RTO=25.0; RTM=33.0;  RQ=2.22
  else
        CTO=20.0; CTM=25.0; CQ=2.22; RTO=25.0; RTM=33.0;  RQ=2.22
  endif
!print*, Ctime, wtemp
...
!---Maximum Consumption
  if (iage(i) .eq. 1) then
       ...
       gcmaxfi(i) = 0.22 * ttt1**0.342* gctemp
    end if
 !print*,  ctime,  gcmaxfi(i)
 grtemp=exp((wtemp-Tm)*log(2.2)/10)
! print*, 'respiration', wtemp,Tm,grtemp
     if (iage(i) .eq. 2) then
       u=9.1*(xfi(i)**0.24)
       elseif (iage(i) .eq. 3) then
       u=5.32*(xfi(i)**0.32)
...
! if (iage(i) .eq. 2) then
! print*, 'consum',gctemp,gcmaxfi(i),grtemp 
! endif
  ! --- multispecies functional response
  ! --- use either this or adjust little p
  ! ---consumption
  cnum=zoo4 * vul(1)/k(1,iage(i))+zoo5*vul(2)/k(2,iage(i))+zoo6*vul(3)/k(3,iage(i))
  c1=gcmaxfi(i)*zoo4*vul(1)/k(1,iage(i))
  c2=gcmaxfi(i)*zoo5*vul(2)/k(2,iage(i))
  c3=gcmaxfi(i)*zoo6*vul(3)/k(3,iage(i))
!
  con1=c1/(1.0+cnum)
  con2=c2/(1.0+cnum)
  con3=c3/(1.0+cnum)
